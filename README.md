...
**Overview**
This project details the mechanical design and computational fluid dynamics (CFD) analysis of an ultralight canister stove. The core objective is to maximise thermal efficiency and validate variable-power flame stability under harsh environmental boundary conditions, specifically a 5 m/s crosswind.
...
*   Simulates the coupled thermal interaction between the solid domains (titanium/aluminum conduction) and fluid domains (combustion gas and crosswind convection).
*   Validates the physical bluff bodies (retention lips) designed to anchor the flame and prevent low-power blowout in a 5 m/s crosswind.

**Engineering Constraints & Real-World Considerations**
*   **Mission-Driven Environment:** The aerodynamic baffle and variable power settings were specifically developed to survive the high-wind, unpredictable environments experienced during self-supported, multi-day wild camping expeditions on the Isle of Arran and Beinn Narnain.
*   **Stiffness-to-Weight Optimization:** Engineered under a strict mass budget of less than 100g, while maintaining the structural rigidity required to safely support a 1.0 kg payload (1 Liter of water) and dynamic stirring forces.
...
*   **Analytical Validation:** Theoretical thermodynamic baseline calculations, mass flow rates, and efficiency limits were modelled using custom data automation scripts in Python and R Studio prior to CAD generation.
